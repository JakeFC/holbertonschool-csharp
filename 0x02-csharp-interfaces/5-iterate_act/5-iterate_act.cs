...
///<summary>Holds IterateAction() method.</summary>
public class RoomObjects
{
	///<summary>Iterates through a list of Base objects and performs relevant actions, based on their interfaces.</summary>
	public static void IterateAction(List<Base> roomObjects, Type type)
	{
		if (type == typeof(IInteractive))
		{
			...
			{
				// Using as for error-free typecasting.
				IInteractive intItem = item as IInteractive;
				if (intItem != null)
					intItem.Interact();
			}
		}
		if (type == typeof(IBreakable))
		{
			foreach (Base item in roomObjects)
			{
				// Storing fitting objects into temporary, typed variables.
				if (item is IBreakable breItem)
					breItem.Break();
			}
		}
		if (type == typeof(ICollectable))
		{
			foreach (Base item in roomObjects)
			{
				if (item is ICollectable colItem)
					colItem.Collect();
			}
		}
	}
